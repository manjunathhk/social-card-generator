...
## History: browser-only, on purpose

The History panel needs the same behaviour whether the page is opened as a file, viewed as a claude.ai artifact, or served by `server/`: source, format, layout and theme overrides, the custom palette if any, the branding fields as typed (not the resolved `Branding`, so a blank field stays blank on reopen), and the exported PNG. It gets that everywhere from one rule: **the server never stores, renders, or validates anything a visitor exported.**

An earlier version of this feature did have a server-side store (`server/store.ts`, one `<id>.json` and one `<id>.png` per card under `<dataDir>/cards/`) fronted by a JSON API. That made history shared across every visitor of a deployed instance — anyone who could reach the URL could list, open, and delete anyone else's cards, because the server had no concept of "whose" a card was and no auth to hang one off of. That's fine for a single person on their own machine, but wrong the moment the URL is reachable by more than one person, which a Docker deployment is built for. So the store was removed rather than gated behind an opt-in flag: an insecure-by-default toggle is the same bug with an extra step.

The client (`web/sandbox/index.template.html`) now only ever talks to its own browser's storage. `HistoryStore` wraps IndexedDB: `list`/`save`/`get`/`remove` read and write an object store keyed by a random id, and `imageUrl` returns an object URL for the stored `Blob`. Nothing here is same-origin-shared or server-visible — clearing that browser's site data is the only way to lose it, and the only way anyone else sees it is if they're looking at the same browser profile.

History is **append-only** by design: `openHistory` loads a record's source back into the editor, but exporting again always creates a new entry rather than updating the one that was opened. A feature called "history" that can silently overwrite itself is a footgun.

## The Docker image

The image has two stages, and the split matters more than usual here: the **build** stage needs the full dependency tree (`shiki`, `@fontsource/*`, `esbuild`, `typescript`) to produce `out/sandbox.html` and compile the server, but the **runtime** stage needs neither those dependencies nor Playwright's browser — rendering happens in the visitor's browser, and the server stores nothing at all. So the runtime stage copies exactly two things out of the build stage, `out/sandbox.html` and `dist-server/`, into a fresh `node:24-slim`, with no `npm ci` and no `node_modules` at all, and no volume — there's nothing on the container that needs to survive a restart. `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` in the build stage stops Playwright's postinstall from downloading Chromium for an image that will never launch a browser.

`SANDBOX_HTML_PATH` and `PORT` are the two knobs `server/index.ts` reads from the environment. The `HEALTHCHECK` calls `/api/health` with Node's own `fetch` rather than installing `curl`, so the final image stays exactly as minimal as the two-file copy above implies.
